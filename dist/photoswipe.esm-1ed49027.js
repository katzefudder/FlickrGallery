...
const U = "cubic-bezier(.4,0,.22,1)";
function R(r, t, i, e) {
  r.style.transition = t ? `${t} ${i}ms ${e || U}` : "none";
}
function L(r, t, i) {
...
class Y {
  ...
  _toggleListener(t, i, e, s, n, o) {
    ...
    i.split(" ").forEach((h) => {
      if (h) {
        o || (n ? this._pool = this._pool.filter((d) => d.type !== h || d.listener !== e || d.target !== t) : this._pool.push({
          target: t,
          type: h,
          listener: e,
          passive: s
...
class $ {
   ...
   */
  _updateAxis(t) {
    const { pswp: i } = this.slide, e = this.slide[t === "x" ? "width" : "height"] * this.currZoomLevel, n = S(
      t === "x" ? "left" : "top",
      i.options,
      i.viewportSize,
      this.slide.data,
...
class k {
  ...
  update(t, i, e) {
    ...
    this.fit = Math.min(1, n < o ? n : o), this.fill = Math.min(1, n > o ? n : o), this.vFill = Math.min(1, o), this.initial = this._getInitial(), this.secondary = this._getSecondary(), this.max = Math.max(
      this.initial,
      this.secondary,
      this._getMax()
    ), this.min = Math.min(
...
class j {
   ...
   */
  constructor(t, i, e) {
    this.data = t, this.index = i, this.pswp = e, this.isActive = i === e.currIndex, this.currentResolution = 0, this.panAreaSize = { x: 0, y: 0 }, this.pan = { x: 0, y: 0 }, this.isFirstSlide = this.isActive && !e.opener.isOpen, this.zoomLevels = new k(e.options, t, i, e), this.pswp.dispatch("gettingData", {
      slide: this,
      data: this.data,
      index: i
    }), this.content = this.pswp.contentLoader.getContentBySlide(this), this.container = m("pswp__zoom-wrap", "div"), this.holderElement = null, this.currZoomLevel = 1, this.width = this.content.width, this.height = this.content.height, this.heavyAppended = !1, this.bounds = new $(this), this.prevDisplayedWidth = -1, this.prevDisplayedHeight = -1, this.pswp.dispatch("slideInit", { slide: this });
  }
  /**
   ...
   */
  appendHeavy() {
    const { pswp: t } = this, i = !0;
    this.heavyAppended || !t.opener.isOpen || t.mainScroll.isShifted() || !this.isActive && !i || this.pswp.dispatch("appendHeavy", { slide: this }).defaultPrevented || (this.heavyAppended = !0, this.content.append(), this.pswp.dispatch("appendHeavyContent", { slide: this }));
  }
  /**
  ...
  getPlaceholderElement() {
    var t;
    return (t = this.content.placeholder) == null ? void 0 : t.element;
  }
  /**
  ...
  _applyZoomTransform(t, i, e) {
    e /= this.currentResolution || this.zoomLevels.initial, v(this.container, t, i, e);
  }
  calculateSize() {
    const { pswp: t } = this;
    u(
      this.panAreaSize,
      N(t.options, t.viewportSize, this.data, this.index)
...
const Q = 0.35, J = 0.6, z = 0.4, O = 0.5;
function tt(r, t) {
  return r * t / (1 - t);
}
class it {
  ...
  start() {
    this.pswp.currSlide && u(this.startPan, this.pswp.currSlide.pan), this.pswp.animations.stopAll();
  }
  change() {
    const { p1: t, prevP1: i, dragAxis: e } = this.gestures, { currSlide: s } = this.pswp;
    ...
    const { velocity: t } = this.gestures, { mainScroll: i, currSlide: e } = this.pswp;
    let s = 0;
    if (this.pswp.animations.stopAll(), i.isShifted()) {
      const o = (i.x - i.getCurrSlideX()) / this.pswp.viewportSize.x;
      t.x < -O && o < 0 || t.x < 0.1 && o < -0.5 ? (s = 1, t.x = Math.min(t.x, 0)) : (t.x > O && o > 0 || t.x > -0.1 && o > 0.5) && (s = -1, t.x = Math.max(t.x, 0)), i.moveIndexBy(s, !0, t.x);
    }
  ...
  _finishPanGestureForAxis(t) {
    const { velocity: i } = this.gestures, { currSlide: e } = this.pswp;
    if (!e)
      return;
    const { pan: s, bounds: n } = e, o = s[t], a = this.pswp.bgOpacity < 1 && t === "y", l = 0.995, h = o + tt(i[t], l);
    if (a) {
      const g = this._getVerticalDragRatio(o), w = this._getVerticalDragRatio(h);
      if (g < 0 && w < -z || g > 0 && w > z) {
        this.pswp.close();
        return;
      }
    }
    const c = n.correctPan(t, h);
    if (o === c)
      return;
    const d = c === h ? 1 : 0.82, p = this.pswp.bgOpacity, _ = c - o;
    this.pswp.animations.startSpring({
      ...
      onUpdate: (g) => {
        if (a && this.pswp.bgOpacity < 1) {
          const w = 1 - (c - g) / _;
          this.pswp.applyBgOpacity(I(
            p + (1 - p) * w,
            0,
            1
          ));
        }
        s[t] = Math.floor(g), e.applyCurrentZoomPan();
      }
    });
  }
  /**
  ...
  _panOrMoveMainScroll(t) {
    const { p1: i, dragAxis: e, prevP1: s, isMultitouch: n } = this.gestures, { currSlide: o, mainScroll: a } = this.pswp, l = i[t] - s[t], h = a.x + l;
    if (!l || !o)
      return !1;
    if (t === "x" && !o.isPannable() && !n)
      return a.moveTo(h, !0), !0;
    const { bounds: c } = o, d = o.pan[t] + l;
    if (this.pswp.options.allowPanToNext && e === "x" && t === "x" && !n) {
      const p = a.getCurrSlideX(), _ = a.x - p, g = l > 0, w = !g;
      if (d > c.min[t] && g) {
  ...
  _getVerticalDragRatio(t) {
    var i;
    return (t - (((i = this.pswp.currSlide) == null ? void 0 : i.bounds.center.y) ?? 0)) / (this.pswp.viewportSize.y / 3);
  }
  /**
...
class nt {
   ...
   */
  constructor(t) {
    this.gestures = t, this._startPan = { x: 0, y: 0 }, this._startZoomPoint = { x: 0, y: 0 }, this._zoomPoint = { x: 0, y: 0 }, this._wasOverFitZoomLevel = !1, this._startZoomLevel = 1;
  }
  start() {
    const { currSlide: t } = this.gestures.pswp;
    t && (this._startZoomLevel = t.currZoomLevel, u(this._startPan, t.pan)), this.gestures.pswp.animations.stopAllPan(), this._wasOverFitZoomLevel = !1;
  }
  change() {
    const { p1: t, startP1: i, p2: e, startP2: s, pswp: n } = this.gestures, { currSlide: o } = n;
    if (!o)
      return;
    const a = o.zoomLevels.min, l = o.zoomLevels.max;
    if (!o.isZoomable() || n.mainScroll.isShifted())
      return;
    E(this._startZoomPoint, i, s), E(this._zoomPoint, t, e);
    let h = 1 / A(i, s) * A(t, e) * this._startZoomLevel;
    if (h > o.zoomLevels.initial + o.zoomLevels.initial / 15 && (this._wasOverFitZoomLevel = !0), h < a)
      if (n.options.pinchToClose && !this._wasOverFitZoomLevel && this._startZoomLevel <= o.zoomLevels.initial) {
        const c = 1 - (a - h) / (a / 1.2);
        n.dispatch("pinchClose", { bgOpacity: c }).defaultPrevented || n.applyBgOpacity(c);
      } else
        h = a - (a - h) * st;
    else
      h > l && (h = l + (h - l) * et);
    o.pan.x = this._calculatePanForZoomLevel("x", h), o.pan.y = this._calculatePanForZoomLevel("y", h), o.setZoomLevel(h), o.applyCurrentZoomPan();
  }
  end() {
   ...
   */
  correctZoomPan(t) {
    const { pswp: i } = this.gestures, { currSlide: e } = i;
    ...
    const a = i.bgOpacity, l = i.bgOpacity < 1, h = u({ x: 0, y: 0 }, e.pan);
    let c = u({ x: 0, y: 0 }, h);
    t && (this._zoomPoint.x = 0, this._zoomPoint.y = 0, this._startZoomPoint.x = 0, this._startZoomPoint.y = 0, this._startZoomLevel = s, u(this._startPan, h)), o && (c = {
    ...
    i.animations.stopAllPan(), i.animations.startSpring({
      ...
      onUpdate: (p) => {
        if (p /= 1e3, d || o) {
          if (d && (e.pan.x = h.x + (c.x - h.x) * p, e.pan.y = h.y + (c.y - h.y) * p), o) {
            const _ = s + (n - s) * p;
            e.setZoomLevel(_);
          }
          e.applyCurrentZoomPan();
        }
        l && i.bgOpacity < 1 && i.applyBgOpacity(I(
          a + (1 - a) * p,
          0,
          1
        ));
...
class ot {
  ...
  _doClickOrTapAction(t, i, e) {
    var l;
    const { pswp: s } = this.gestures, { currSlide: n } = s, o = (
      /** @type {AddPostfix<Actions, 'Action'>} */
      t + "Action"
    ), a = s.options[o];
    if (!s.dispatch(o, { point: i, originalEvent: e }).defaultPrevented) {
      ...
      switch (a) {
        ...
        case "next":
          ...
          break;
        case "zoom":
          n == null || n.toggleZoom(i);
          break;
        case "zoom-or-close":
          n != null && n.isZoomable() && n.zoomLevels.secondary !== n.zoomLevels.initial ? n.toggleZoom(i) : s.options.clickToCloseNonZoomable && s.close();
          break;
...
class lt {
   ...
   */
  _bindEvents(t, i, e, s) {
    const { pswp: n } = this, { events: o } = n, a = s ? t + s : "";
    o.add(
      n.scrollWrap,
      t + i,
      /** @type EventListener */
   ...
   */
  onPointerUp(t) {
    this._numActivePoints && (this._updatePoints(t, "up"), !this.pswp.dispatch("pointerUp", { originalEvent: t }).defaultPrevented && (this._numActivePoints === 0 && (this._rafStopLoop(), this.isDragging ? this._finishDrag() : !this.isZooming && !this.isMultitouch && this._finishTap(t)), this._numActivePoints < 2 && this.isZooming && (this.isZooming = !1, this.zoomLevels.end(), this._numActivePoints === 1 && (this.dragAxis = null, this._updateStartPoints()))));
  }
  /**
   * @private
   */
  ...
  _updateVelocity(t) {
    ...
    e < 50 && !t || (this.velocity.x = this._getVelocity("x", e), this.velocity.y = this._getVelocity("y", e), this._intervalTime = i, u(this._intervalP1, this.p1), this._velocityCalculated = !0);
  }
  /**
   * @private
   * @param {PointerEvent} e
  ...
  _finishTap(t) {
    ...
    this._tapTimer ? (this._clearTapTimer(), A(this._lastStartP1, this.startP1) < ht && this.tapHandler.doubleTap(this.startP1, t)) : (u(this._lastStartP1, this.startP1), this._tapTimer = setTimeout(() => {
  ...
  _updatePoints(t, i) {
    if (this._pointerEventEnabled) {
      ...
      i === "up" && s > -1 ? this._ongoingPointers.splice(s, 1) : i === "down" && s === -1 ? this._ongoingPointers.push(this._convertEventPosToPoint(e, { x: 0, y: 0 })) : s > -1 && this._convertEventPosToPoint(e, this._ongoingPointers[s]), this._numActivePoints = this._ongoingPointers.length, this._numActivePoints > 0 && u(this.p1, this._ongoingPointers[0]), this._numActivePoints > 1 && u(this.p2, this._ongoingPointers[1]);
    } else {
      const e = (
        /** @type {TouchEvent} */
        t
...
class dt {
   ...
   */
  resize(t) {
    const { pswp: i } = this, e = Math.round(
    ...
    s && (this.slideWidth = e, this.moveTo(this.getCurrSlideX())), this.itemHolders.forEach((n, o) => {
      s && v(n.el, (o + this._containerShiftIndex) * this.slideWidth), t && n.slide && n.slide.resize();
    });
  }
  /**
  ...
  appendHolders() {
    this.itemHolders = [];
    for (let t = 0; t < 3; t++) {
      const i = m("pswp__item", "div", this.pswp.container);
  ...
  updateCurrItem() {
    var n;
    const { pswp: t } = this, i = this._prevPositionIndex - this._currPositionIndex;
    if (!i)
      return;
    this._prevPositionIndex = this._currPositionIndex, t.currIndex = t.potentialIndex;
    let e = Math.abs(i), s;
    e >= 3 && (this._containerShiftIndex += i + (i > 0 ? -3 : 3), e = 3);
    for (let o = 0; o < e; o++)
      i > 0 ? (s = this.itemHolders.shift(), s && (this.itemHolders[2] = s, this._containerShiftIndex++, v(s.el, (this._containerShiftIndex + 2) * this.slideWidth), t.setContent(s, t.currIndex - e + o + 2))) : (s = this.itemHolders.pop(), s && (this.itemHolders.unshift(s), this._containerShiftIndex--, v(s.el, this._containerShiftIndex * this.slideWidth), t.setContent(s, t.currIndex + e - o - 2)));
    Math.abs(this._containerShiftIndex) > 50 && !this.isShifted() && (this.resetPosition(), this.resize()), t.animations.stopAllPan(), this.itemHolders.forEach((o, a) => {
      o.slide && o.slide.setIsActive(a === 1);
    }), t.currSlide = (n = this.itemHolders[1]) == null ? void 0 : n.slide, t.contentLoader.updateLazy(i), t.currSlide && t.currSlide.applyCurrentZoomPan(), t.dispatch("change");
  }
  /**
  ...
  moveTo(t, i) {
    ...
    this.x = t, this.pswp.container && v(this.pswp.container, t), this.pswp.dispatch("moveMainScroll", { x: t, dragging: i ?? !1 });
  }
}
const pt = {
  Escape: 27,
...
class ut {
  ...
  constructor(t) {
    this.pswp = t, this._wasFocused = !1, t.on("bindEvents", () => {
      t.options.initialPointerPos || this._focusRoot(), t.events.add(
        document,
        "focusin",
        /** @type EventListener */
  ...
  _onKeyDown(t) {
    const { pswp: i } = this;
    if (i.dispatch("keydown", { originalEvent: t }).defaultPrevented || K(t))
      return;
    let e, s, n = !1;
    const o = "key" in t;
...
class ft {
  ...
  constructor(t) {
    ...
    this._target = i, this._onComplete = e, this._finished = !1, this._onTransitionEnd = this._onTransitionEnd.bind(this), this._helperTimeout = setTimeout(() => {
      R(i, l, o, a), this._helperTimeout = setTimeout(() => {
...
class yt {
   ...
   */
  constructor(t, i, e) {
    this.velocity = t * 1e3, this._dampingRatio = i || gt, this._naturalFrequency = e || _t, this._dampedFrequency = this._naturalFrequency, this._dampingRatio < 1 && (this._dampedFrequency *= Math.sqrt(1 - this._dampingRatio * this._dampingRatio));
  }
  /**
...
class vt {
  ...
  constructor(t) {
    ...
    this.onFinish = a;
    const c = new yt(s, l, h);
    let d = Date.now(), p = i - e;
    const _ = () => {
      this._raf && (p = c.easeFrame(p, Date.now() - d), Math.abs(p) < 1 && Math.abs(c.velocity) < 50 ? (n(e), o && o(), this.onFinish()) : (d = Date.now(), n(p + e), this._raf = requestAnimationFrame(_)));
    };
  ...
  _onWheel(t) {
    ...
    if (i && !this.pswp.dispatch("wheel", { originalEvent: t }).defaultPrevented)
      if (t.ctrlKey || this.pswp.options.wheelToZoom) {
        ...
        i.isPannable() && (t.deltaMode === 1 && (e *= 18, s *= 18), i.panTo(
          i.pan.x - e,
          i.pan.y - s
        ));
  }
...
class xt {
   ...
   */
  constructor(t, i) {
    const e = i.name || i.className;
    let s = i.html;
    if (t.options[e] === !1)
      return;
    typeof t.options[e + "SVG"] == "string" && (s = t.options[e + "SVG"]), t.dispatch("uiElementCreate", { data: i });
    let n = "";
    i.isButton ? (n += "pswp__button ", n += i.className || `pswp__button--${i.name}`) : n += i.className || `pswp__${i.name}`;
    let o = i.isButton ? i.tagName || "button" : i.tagName || "div";
    o = /** @type {keyof HTMLElementTagNameMap} */
    o.toLowerCase();
    const a = m(n, o);
    if (i.isButton) {
      ...
      let { title: c } = i;
      const { ariaLabel: d } = i;
      typeof t.options[e + "Title"] == "string" && (c = t.options[e + "Title"]), c && (a.title = c);
      const p = d || c;
      p && a.setAttribute("aria-label", p);
    }
    a.innerHTML = St(s), i.onInit && i.onInit(a, t), i.onClick && (a.onclick = (c) => {
      typeof i.onClick == "string" ? t[i.onClick]() : typeof i.onClick == "function" && i.onClick(c, a, t);
    });
    const l = i.appendTo || "bar";
    let h = t.element;
    l === "bar" ? (t.topBar || (t.topBar = m("pswp__top-bar pswp__hide-on-close", "div", t.scrollWrap)), h = t.topBar) : (a.classList.add("pswp__hide-on-close"), l === "wrapper" && (h = t.scrollWrap)), h == null || h.appendChild(t.applyFilters("uiElement", a, i));
  }
}
function H(r, t, i) {
...
const bt = {
  ...
  onInit: (r, t) => {
    let i, e = null;
    const s = (a, l) => {
      r.classList.toggle("pswp__preloader--" + a, l);
    }, n = (a) => {
      i !== a && (i = a, s("active", a));
    }, o = () => {
      var a;
      if (!((a = t.currSlide) != null && a.content.isLoading())) {
        n(!1), e && (clearTimeout(e), e = null);
        return;
      }
      e || (e = setTimeout(() => {
        var l;
        n(!!((l = t.currSlide) != null && l.content.isLoading())), e = null;
...
class zt {
  ...
  init() {
    ...
    this.isRegistered = !1, this.uiElementsData = [
      ...
      Ct,
      Tt
    ], t.dispatch("uiRegister"), this.uiElementsData.sort((i, e) => (i.order || 0) - (e.order || 0)), this.items = [], this.isRegistered = !0, this.uiElementsData.forEach((i) => {
      this.registerElement(i);
    }), t.on("change", () => {
      var i;
      (i = t.element) == null || i.classList.toggle("pswp--one-slide", t.getNumItems() === 1);
...
function Et(r, t, i) {
  const e = r.getBoundingClientRect(), s = e.width / t, n = e.height / i, o = s > n ? s : n, a = (e.width - t * o) / 2, l = (e.height - i * o) / 2, h = {
    x: e.left + a,
    y: e.top + l,
    w: t * o
  };
...
function Zt(r, t, i) {
  ...
  return o = i.applyFilters("thumbEl", o, t, r), o && (t.thumbCropped ? n = Et(
    o,
    t.width || t.w || 0,
    t.height || t.h || 0
  ) : n = Ot(o)), i.applyFilters("thumbBounds", n, t, r);
}
class Dt {
  /**
   * @param {T} type
...
class Mt {
  ...
  addFilter(t, i, e = 100) {
    var s, n, o;
    this._filters[t] || (this._filters[t] = []), (s = this._filters[t]) == null || s.push({ fn: i, priority: e }), (n = this._filters[t]) == null || n.sort((a, l) => a.priority - l.priority), (o = this.pswp) == null || o.addFilter(t, i, e);
  }
  /**
   * @template {keyof PhotoSwipeFiltersMap} T
   * @param {T} name
  ...
  applyFilters(t, ...i) {
    var e;
    return (e = this._filters[t]) == null || e.forEach((s) => {
  ...
  on(t, i) {
    var e, s;
    this._listeners[t] || (this._listeners[t] = []), (e = this._listeners[t]) == null || e.push(i), (s = this.pswp) == null || s.on(t, i);
  }
  /**
   * @template {keyof PhotoSwipeEventsMap} T
   * @param {T} name
  ...
  off(t, i) {
    var e;
    this._listeners[t] && (this._listeners[t] = this._listeners[t].filter((s) => i !== s)), (e = this.pswp) == null || e.off(t, i);
  }
  /**
   * @template {keyof PhotoSwipeEventsMap} T
   * @param {T} name
  ...
  dispatch(t, i) {
    ...
    return (s = this._listeners[t]) == null || s.forEach((n) => {
      n.call(this, e);
    }), e;
  }
}
class Rt {
  ...
  destroy() {
    var t;
    (t = this.element) != null && t.parentNode && this.element.remove(), this.element = null;
  }
}
class Ft {
  /**
  ...
  load(t, i) {
    if (this.slide && this.usePlaceholder())
      if (this.placeholder) {
        ...
        this.placeholder = new Rt(
          e,
          this.slide.container
        );
      }
    this.element && !i || this.instance.dispatch("contentLoad", { content: this, isLazy: t }).defaultPrevented || (this.isImageContent() ? (this.element = m("pswp__img", "img"), this.displayedImageWidth && this.loadImage(t)) : (this.element = m("pswp__content", "div"), this.element.innerHTML = this.data.html || ""), i && this.slide && this.slide.updateContentSize(!0));
  }
  /**
   * Preload image
   *
   ...
   */
  loadImage(t) {
    if (!this.isImageContent() || !this.element || this.instance.dispatch("contentLoadImage", { content: this, isLazy: t }).defaultPrevented)
      return;
    const i = (
      /** @type HTMLImageElement */
      this.element
    );
    this.updateSrcsetSizes(), this.data.srcset && (i.srcset = this.data.srcset), i.src = this.data.src ?? "", i.alt = this.data.alt ?? "", this.state = f.LOADING, i.complete ? this.onLoaded() : (i.onload = () => {
      this.onLoaded();
    }, i.onerror = () => {
      this.onError();
    });
   ...
   */
  onLoaded() {
    this.state = f.LOADED, this.slide && this.element && (this.instance.dispatch("loadComplete", { slide: this.slide, content: this }), this.slide.isActive && this.slide.heavyAppended && !this.element.parentNode && (this.append(), this.slide.updateContentSize(!0)), (this.state === f.LOADED || this.state === f.ERROR) && this.removePlaceholder());
  }
  /**
   ...
   */
  setDisplayedSize(t, i) {
    if (this.element && (this.placeholder && this.placeholder.setDisplayedSize(t, i), !this.instance.dispatch(
      "contentResize",
      { content: this, width: t, height: i }
    ).defaultPrevented && (L(this.element, t, i), this.isImageContent() && !this.isError()))) {
      const e = !this.displayedImageWidth && t;
      this.displayedImageWidth = t, this.displayedImageHeight = i, e ? this.loadImage(!1) : this.updateSrcsetSizes(), this.slide && this.instance.dispatch(
        "imageSizeChange",
        { slide: this.slide, width: t, height: i, content: this }
      );
    }
  }
  /**
  ...
  displayError() {
    ...
    if (this.slide) {
      ...
      i.innerText = ((t = this.instance.options) == null ? void 0 : t.errorMsg) ?? "", i = /** @type {HTMLDivElement} */
      this.instance.applyFilters(
        "contentErrorElement",
        i,
        this
      ), this.element = m("pswp__content pswp__error-msg-container", "div"), this.element.appendChild(i), this.slide.container.innerText = "", this.slide.container.appendChild(this.element), this.slide.updateContentSize(!0), this.removePlaceholder();
    }
  }
  /**
   * Append the content
   ...
   */
  activate() {
    this.instance.dispatch("contentActivate", { content: this }).defaultPrevented || !this.slide || (this.isImageContent() && this.isDecoding && !C() ? this.appendImage() : this.isError() && this.load(!1, !0), this.slide.holderElement && this.slide.holderElement.setAttribute("aria-hidden", "false"));
  }
  /**
...
function W(r, t, i) {
  ...
  return e.lazyLoad(), s && e.setDisplayedSize(
...
class kt {
  ...
  updateLazy(t) {
    ...
    if (i.dispatch("lazyLoad").defaultPrevented)
      return;
    const { preload: e } = i.options, s = t === void 0 ? !0 : t >= 0;
    let n;
    for (n = 0; n <= e[1]; n++)
...
class Ht extends Mt {
  ...
  getNumItems() {
    ...
    const e = this.dispatch("numItems", {
      dataSource: i,
      numItems: t
    });
  ...
  getItemData(t) {
    var o;
    const i = (o = this.options) == null ? void 0 : o.dataSource;
    let e = {};
    Array.isArray(i) ? e = i[t] : i && "gallery" in i && (i.items || (i.items = this._getGalleryDOMElements(i.gallery)), e = i.items[t]);
    let s = e;
  ...
  _getGalleryDOMElements(t) {
    var i, e;
    return (i = this.options) != null && i.children || (e = this.options) != null && e.childSelector ? X(
  ...
  _domElementToItemData(t) {
    ...
    if (e) {
      ...
      s && (i.msrc = s.currentSrc || s.src, i.alt = s.getAttribute("alt") ?? ""), (e.dataset.pswpCropped || e.dataset.cropped) && (i.thumbCropped = !0);
    }
...
class Wt {
  ...
  _applyStartProps() {
    var s;
    const { pswp: t } = this, i = this.pswp.currSlide, { options: e } = t;
  ...
  _initiate() {
    var t, i;
    (t = this.pswp.element) == null || t.style.setProperty("--pswp-transition-duration", this._duration + "ms"), this.pswp.dispatch(
      this.isOpening ? "openingAnimationStart" : "closingAnimationStart"
    ), this.pswp.dispatch(
      /** @type {'initialZoomIn' | 'initialZoomOut'} */
      "initialZoom" + (this.isOpening ? "In" : "Out")
    ), (i = this.pswp.element) == null || i.classList.toggle("pswp--ui-visible", this.isOpening), this.isOpening ? (this._placeholder && (this._placeholder.style.opacity = "1"), this._animateToOpenState()) : this.isClosing && this._animateToClosedState(), this._useAnimation || this._onAnimationComplete();
  }
  /** @private */
  _onAnimationComplete() {
    ...
    ), this.isClosed ? t.destroy() : this.isOpen && (this._animateZoom && t.container && (t.container.style.overflow = "visible", t.container.style.width = "100%"), (i = t.currSlide) == null || i.applyCurrentZoomPan());
  }
  /** @private */
  _animateToOpenState() {
    const { pswp: t } = this;
    this._animateZoom && (this._croppedZoom && this._cropContainer1 && this._cropContainer2 && (this._animateTo(this._cropContainer1, "transform", "translate3d(0,0,0)"), this._animateTo(this._cropContainer2, "transform", "none")), t.currSlide && (t.currSlide.zoomAndPanToInitial(), this._animateTo(
      t.currSlide.container,
      "transform",
      t.currSlide.getCurrentTransform()
    ))), this._animateBgOpacity && t.bg && this._animateTo(t.bg, "opacity", String(t.options.bgOpacity)), this._animateRootOpacity && t.element && this._animateTo(t.element, "opacity", "1");
  }
  /** @private */
  ...
  _setClosedStateZoomPan(t) {
    if (!this._thumbBounds)
      return;
    const { pswp: i } = this, { innerRect: e } = this._thumbBounds, { currSlide: s, viewportSize: n } = i;
    if (this._croppedZoom && e && this._cropContainer1 && this._cropContainer2) {
      const o = -n.x + (this._thumbBounds.x - e.x) + e.w, a = -n.y + (this._thumbBounds.y - e.y) + e.h, l = n.x - e.w, h = n.y - e.h;
      t ? (this._animateTo(
        ...
        "transform",
        b(l, h)
      )) : (v(this._cropContainer1, o, a), v(this._cropContainer2, l, h));
    }
    s && (u(s.pan, e || this._thumbBounds), s.currZoomLevel = this._thumbBounds.w / s.width, t ? this._animateTo(s.container, "transform", s.getCurrentTransform()) : s.applyCurrentZoomPan());
  }
  /**
   * @private
  ...
  _animateTo(t, i, e) {
    ...
    const { animations: s } = this.pswp, n = {
...
class Ut extends Ht {
   ...
   */
  constructor(t) {
    super(), this.options = this._prepareOptions(t || {}), this.offset = { x: 0, y: 0 }, this._prevViewportSize = { x: 0, y: 0 }, this.viewportSize = { x: 0, y: 0 }, this.bgOpacity = 1, this.currIndex = 0, this.potentialIndex = 0, this.isOpen = !1, this.isDestroying = !1, this.hasMouse = !1, this._initialItemData = {}, this._initialThumbBounds = void 0, this.topBar = void 0, this.element = void 0, this.template = void 0, this.container = void 0, this.scrollWrap = void 0, this.currSlide = void 0, this.events = new Y(), this.animations = new wt(), this.mainScroll = new dt(this), this.gestures = new lt(this), this.opener = new Wt(this), this.keyboard = new ut(this), this.contentLoader = new kt(this);
  }
  /** @returns {boolean} */
  init() {
  ...
  zoomTo(...t) {
    var i;
    (i = this.currSlide) == null || i.zoomTo(...t);
  }
  /**
  ...
  toggleZoom() {
    var t;
    (t = this.currSlide) == null || t.toggleZoom();
  }
  /**
   * Close the gallery.
  ...
  destroy() {
    ...
    this.dispatch("destroy"), this._listeners = {}, this.scrollWrap && (this.scrollWrap.ontouchmove = null, this.scrollWrap.ontouchend = null), (t = this.element) == null || t.remove(), this.mainScroll.itemHolders.forEach((i) => {
      var e;
      (e = i.slide) == null || e.destroy();
  ...
  refreshSlideContent(t) {
    this.contentLoader.removeByIndex(t), this.mainScroll.itemHolders.forEach((i, e) => {
      var n, o;
      let s = (((n = this.currSlide) == null ? void 0 : n.index) ?? 0) - 1 + e;
      this.canLoop() && (s = this.getLoopedIndex(s)), s === t && (this.setContent(i, t, !0), e === 1 && (this.currSlide = i.slide, (o = i.slide) == null || o.setIsActive(!0)));
  ...
  updateSize(t) {
    ...
    !t && x(i, this._prevViewportSize) || (u(this._prevViewportSize, i), this.dispatch("beforeResize"), u(this.viewportSize, this._prevViewportSize), this._updatePageScrollOffset(), this.dispatch("viewportSize"), this.mainScroll.resize(this.opener.isOpen), !this.hasMouse && window.matchMedia("(any-hover: hover)").matches && this.mouseDetected(), this.dispatch("resize"));
  }
  /**
  ...
  mouseDetected() {
    var t;
    this.hasMouse || (this.hasMouse = !0, (t = this.element) == null || t.classList.add("pswp--has_mouse"));
  }
  /**
  ...
  _createMainStructure() {
    this.element = m("pswp", "div"), this.element.setAttribute("tabindex", "-1"), this.element.setAttribute("role", "dialog"), this.template = this.element, this.bg = m("pswp__bg", "div", this.element), this.scrollWrap = m("pswp__scroll-wrap", "section", this.element), this.container = m("pswp__container", "div", this.scrollWrap), this.scrollWrap.setAttribute("aria-roledescription", "carousel"), this.container.setAttribute("aria-live", "off"), this.container.setAttribute("id", "pswp__items"), this.mainScroll.appendHolders(), this.ui = new zt(this), this.ui.init(), (this.options.appendToEl || document.body).appendChild(this.element);
  }
  /**
